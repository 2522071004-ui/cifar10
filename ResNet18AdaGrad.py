...
writer = SummaryWriter(log_dir='logs/AdaGrad')
...
for epoch in tqdm(range(1, n_epochs + 1)):
    ...
    if counter / 10 == 1:
        counter = 0
        lr = lr * 0.5
    optimizer = optim.Adagrad(model.parameters(), lr=lr, lr_decay=0, weight_decay=1e-5, initial_accumulator_value=0, eps=1e-10)
    model.train()
    ###################
    # 训练集的模型 #
    ###################
    ...
    if valid_loss <= valid_loss_min:
        ...
        torch.save(model.state_dict(), 'model/resnet18_cifar10AdaGrad.pt')
