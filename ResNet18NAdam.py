...
writer = SummaryWriter(log_dir='logs/NAdam')
...
for epoch in tqdm(range(1, n_epochs + 1)):
    ...
    if counter / 10 == 1:
        counter = 0
        lr = lr * 0.5
    optimizer = optim.NAdam(model.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, momentum_decay=4e-3, foreach=None)
    ###################
    # 训练集的模型 #
    ###################
    ...
    writer.add_scalar('Acc', right_sample / total_sample, epoch)

    # 如果验证集损失函数减少，就保存模型。
    if valid_loss <= valid_loss_min:
        print('Validation loss decreased ({:.6f} --> {:.6f}).  Saving model ...'.format(valid_loss_min, valid_loss))
        torch.save(model.state_dict(), 'model/resnet18_cifar10NAdam.pt')
